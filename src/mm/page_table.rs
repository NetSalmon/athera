...
pub fn identity_map() -> MemResult<()> {
    ...
    let fw_cfg_range = match unsafe { fdt::Fdt::from_ptr(FDT_ADDR) } {
        Ok(fdt) => FwCfg::probe(&fdt)
            .map(|fw| fw.device.mmio.start..fw.device.mmio.start + fw.device.mmio.size),
        Err(_) => None,
    };
    ...
    // DeviceManager 会从 FDT 建立驱动并访问设备寄存器，因此所有设备 MMIO
    // 区间都必须在初始化设备管理器前建立映射。
    if let Ok(fdt) = unsafe { fdt::Fdt::from_ptr(FDT_ADDR) } {
        for node in fdt.all_nodes() {
            let is_mmio_device = node
                .compatible()
                .map(|compatible| {
                    compatible
                        .all()
                        .any(|value| value == "virtio,mmio" || value == "ns16550a")
                })
                .unwrap_or(false);
            if !is_mmio_device {
                continue;
...
impl PageTableManager {
    ...
    #[inline]
    pub fn flush() {
        crate::arch::riscv64::address_translation_fence();
    }
}
