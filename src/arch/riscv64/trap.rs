...
use crate::{
    ...
    task::scheduler::{save_current, switch},
    user_trap_entry,
};
...
fn trap_handler(
    ...
    match trap {
        ...
        Trap::Exception(Exception::U_MODE_ECALL) => {
            ...
            match syscall::handle(sepc, trap_context) {
                SyscallResult::Return(ret, next) => {
                    ...
                    crate::arch::riscv64::registers::csr::Sepc::write(next);
                }
                // 直接让出CPU
                SyscallResult::Yield => {
        ...
        Trap::Exception(Exception::BREAKPOINT) => {
            debug!("breakpoint");
            crate::arch::riscv64::registers::csr::Sepc::write(sepc + 4);
        }
...
pub fn set_next_timer(gap: Option<u64>) {
    ...
    let t = crate::arch::riscv64::registers::csr::Time::read();
    crate::arch::riscv64::registers::csr::Stimecmp::write(t + gap.unwrap_or(GAP));
}
...
pub fn restore_context(context: &TrapContext) {
    ...
    unsafe {
        asm!(
            ...
            # restore_context does not return through user_trap_entry, so reset
            # the kernel trap stack instead of preserving the current Rust frame.
