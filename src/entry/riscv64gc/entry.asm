# 内核启动汇编。
#
# 启动流程（见 README「初始化依赖」）：
# 1. _start 设置内核栈，把 a1（设备树地址）写入 FDT_ADDR 槽位；
# 2. 设置 stvec = trap_entry、开启 SIE 相应中断位，并置位 SSTATUS
#    (SIE/SUM) 后调用 Rust 侧 main；
# 3. trap_entry 保存/恢复全部通用寄存器（256 字节陷阱帧），把
#    scause/sepc/stval/sstatus/satp 与帧指针作为参数调用 trap_handler。

.section .text.entry
.global _start
.balign 4
_start:
    # 1. 内核栈 + 保存设备树地址
    la sp, kernel_stack_top

    la t0, FDT_ADDR
    sd a1, 0(t0)              # a1 = 设备树物理地址（QEMU 约定）
    mv tp, a0

# 2. 陷阱向量 + 中断掩码
    la t0, trap_entry
    andi t0, t0, ~3
    csrs stvec, t0           # stvec = trap_entry（低 2 位清 0 = DIRECT）

    # SIE: S-Software(1) | S-Timer(5) | S-External(9)
    li   t0, (1 << 1) | (1 << 5) | (1 << 9)
    csrw sie, t0

    # SSTATUS: 先只开启 SUM。SIE 要等 Rust 完成定时器和初始任务
    # 初始化后再开启，避免启动时已有 pending timer 进入空调度器。
    li   t0, (1 << 18)
    csrs sstatus, t0

    call main                # 进入 Rust 内核入口（不返回）

.section .text
.global hart_entry
hart_entry:
    mv tp, a0
hart_entry_loop:
    wfi
    j hart_entry_loop

.section .text
.balign 4
.global trap_entry
trap_entry:
    # 保存 32 个通用寄存器到内核栈（256 字节陷阱帧）
    addi sp, sp, -256
    sd x0, 0(sp)
    sd x1, 8(sp)
    sd x2, 16(sp)
    sd x3, 24(sp)
    sd x4, 32(sp)
    sd x5, 40(sp)
    sd x6, 48(sp)
    sd x7, 56(sp)
    sd x8, 64(sp)
    sd x9, 72(sp)
    sd x10, 80(sp)
    sd x11, 88(sp)
    sd x12, 96(sp)
    sd x13, 104(sp)
    sd x14, 112(sp)
    sd x15, 120(sp)
    sd x16, 128(sp)
    sd x17, 136(sp)
    sd x18, 144(sp)
    sd x19, 152(sp)
    sd x20, 160(sp)
    sd x21, 168(sp)
    sd x22, 176(sp)
    sd x23, 184(sp)
    sd x24, 192(sp)
    sd x25, 200(sp)
    sd x26, 208(sp)
    sd x27, 216(sp)
    sd x28, 224(sp)
    sd x29, 232(sp)
    sd x30, 240(sp)
    sd x31, 248(sp)

    # 传参给 trap_handler(scause, sepc, stval, sstatus, satp, sp)
    csrr a0, scause
    csrr a1, sepc
    csrr a2, stval
    csrr a3, sstatus
    csrr a4, satp
    mv   a5, sp

    call trap_handler

    # 恢复现场
    ld x0, 0(sp)
    ld x1, 8(sp)
    ld x2, 16(sp)
    ld x3, 24(sp)
    ld x4, 32(sp)
    ld x5, 40(sp)
    ld x6, 48(sp)
    ld x7, 56(sp)
    ld x8, 64(sp)
    ld x9, 72(sp)
    ld x10, 80(sp)
    ld x11, 88(sp)
    ld x12, 96(sp)
    ld x13, 104(sp)
    ld x14, 112(sp)
    ld x15, 120(sp)
    ld x16, 128(sp)
    ld x17, 136(sp)
    ld x18, 144(sp)
    ld x19, 152(sp)
    ld x20, 160(sp)
    ld x21, 168(sp)
    ld x22, 176(sp)
    ld x23, 184(sp)
    ld x24, 192(sp)
    ld x25, 200(sp)
    ld x26, 208(sp)
    ld x27, 216(sp)
    ld x28, 224(sp)
    ld x29, 232(sp)
    ld x30, 240(sp)
    ld x31, 248(sp)

    addi sp, sp, 256

    sret                     # 返回被打断的执行流（sepc 可能已被改写）

.section .text
.balign 4
.global user_trap_entry
user_trap_entry:
    csrrw sp, sscratch, sp
    addi sp, sp, -256
    sd x0, 0(sp)
    sd x1, 8(sp)
    sd x2, 16(sp)
    sd x3, 24(sp)
    sd x4, 32(sp)
    sd x5, 40(sp)
    sd x6, 48(sp)
    sd x7, 56(sp)
    sd x8, 64(sp)
    sd x9, 72(sp)
    sd x10, 80(sp)
    sd x11, 88(sp)
    sd x12, 96(sp)
    sd x13, 104(sp)
    sd x14, 112(sp)
    sd x15, 120(sp)
    sd x16, 128(sp)
    sd x17, 136(sp)
    sd x18, 144(sp)
    sd x19, 152(sp)
    sd x20, 160(sp)
    sd x21, 168(sp)
    sd x22, 176(sp)
    sd x23, 184(sp)
    sd x24, 192(sp)
    sd x25, 200(sp)
    sd x26, 208(sp)
    sd x27, 216(sp)
    sd x28, 224(sp)
    sd x29, 232(sp)
    sd x30, 240(sp)
    sd x31, 248(sp)

    csrr t0, sscratch
    sd   t0, 16(sp)

    csrr a0, scause
    csrr a1, sepc
    csrr a2, stval
    csrr a3, sstatus
    csrr a4, satp
    mv   a5, sp

    la t0, trap_entry
    csrw stvec, t0

    call trap_handler

    # 恢复现场
    # 注意：16(sp) 槽位存的是用户栈指针，必须最后恢复 sp；否则后续
    # ld 会以用户栈为基址读取，导致读到错误数据甚至触发页错误。
    addi t0, sp, 256        # t0 = 内核栈顶
    csrw sscratch, t0       # sscratch 恢复为内核栈顶，供下次陷入使用
    la   t0, user_trap_entry
    csrw stvec, t0          # stvec 恢复为用户陷阱向量（否则下次陷入会进 trap_entry）
    ld x0, 0(sp)
    ld x1, 8(sp)
    ld x3, 24(sp)
    ld x4, 32(sp)
    ld x5, 40(sp)
    ld x6, 48(sp)
    ld x7, 56(sp)
    ld x8, 64(sp)
    ld x9, 72(sp)
    ld x10, 80(sp)
    ld x11, 88(sp)
    ld x12, 96(sp)
    ld x13, 104(sp)
    ld x14, 112(sp)
    ld x15, 120(sp)
    ld x16, 128(sp)
    ld x17, 136(sp)
    ld x18, 144(sp)
    ld x19, 152(sp)
    ld x20, 160(sp)
    ld x21, 168(sp)
    ld x22, 176(sp)
    ld x23, 184(sp)
    ld x24, 192(sp)
    ld x25, 200(sp)
    ld x26, 208(sp)
    ld x27, 216(sp)
    ld x28, 224(sp)
    ld x29, 232(sp)
    ld x30, 240(sp)
    ld x31, 248(sp)
    ld t0, 40(sp)           # 恢复 t0（此时 sp 仍是内核帧基址）
    ld x2, 16(sp)           # 最后恢复 sp（用户栈指针）

    sret


.section .data
# FDT_ADDR：启动时由 _start 写入设备树物理地址，Rust 侧作为
# extern 符号读取（见 constants/symbols.rs）。
.global FDT_ADDR
.align 3
FDT_ADDR:
    .quad 0

.section .bss.stack
.global boot_stack_lower_bound
kernel_stack_lower_bound:
    .space 4096 * 16         # 内核启动栈 64 KiB
    .global kernel_stack_top
kernel_stack_top:
