...
use crate::{
    ...
    constants::{MAX_CPU, PAGE_SIZE, TID_MAX},
    error,
...
///
/// 组合各部分的克隆：[`Task::try_clone`] 深拷贝地址空间与
...
impl Tasks {
    ...
    pub fn add(&mut self, tid: Tid, parent: Option<Tid>, tcb: Task) {
    ...
    pub fn select_next(&mut self) -> Result<TaskContext, Error> {
        let next = |tasks: &BTreeMap<Tid, Task>, start| {
            tasks
                .range(start)
